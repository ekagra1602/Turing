(
  ...
  function highlightElement(element, index, parentIframe = null) {
    ...
    try {
      ...
      // const backgroundColor = `rgba(${r}, ${g}, ${b}, 0.0)`; // 0.1 = 10% opacity

      // Create highlight overlay
      const overlay = document.createElement("div");
      overlay.style.position = "fixed";
      overlay.style.border = `2px dashed ${baseColor}`;
  ...
  function isElementAccepted(element) {
    ...
    const alwaysAccept = new Set(["body", "div", "main", "article", "section", "nav", "header", "footer", "label"]);
  ...
  function isInteractiveElement(element) {
    ...
    if (isCookieBannerElement) {
      ...
      if (
        ...
        (element.classList && (element.classList.contains("ot-sdk-button") || element.classList.contains("accept-button") || element.classList.contains("reject-button"))) ||
    ...
    const interactiveParts = new Set(["indicator"]);
    ...
    const hasAddressInputClass =
      element.classList &&
      (element.classList.contains("address-input__container__input") || element.classList.contains("nav-btn") || element.classList.contains("pull-left") || element.classList.contains("btn") || element.classList.contains("btn-secondary"));

    // Added enhancement to capture dropdown interactive elements
    if (element.classList && (element.classList.contains("dropdown-toggle") || element.getAttribute("data-toggle") === "dropdown" || element.getAttribute("aria-haspopup") === "true")) {
      return true;
    }
    ...
    const isCookieBanner =
      (typeof element.id === "string" && (element.id.toLowerCase().includes("cookie") || element.id.toLowerCase().includes("consent") || element.id.toLowerCase().includes("notice"))) ||
      (element.classList && (element.classList.contains("otCenterRounded") || element.classList.contains("ot-sdk-container"))) ||
      ...
      (element.tagName.toLowerCase() === "div" &&
        ((typeof element.id === "string" && element.id.includes("onetrust")) || (element.classList && (element.classList.contains("onetrust") || element.classList.contains("cookie") || element.classList.contains("consent")))));
  ...
  function isInExpandedViewport(element, viewportExpansion) {
    ...
    // Simple viewport check without scroll calculations
    return !(rect.bottom < -viewportExpansion || rect.top > window.innerHeight + viewportExpansion || rect.right < -viewportExpansion || rect.left > window.innerWidth + viewportExpansion);
  }
  ...
  function isInteractiveCandidate(element) {
    ...
    const hasQuickInteractiveAttr = element.hasAttribute("onclick") || element.hasAttribute("role") || element.hasAttribute("tabindex") || element.hasAttribute("aria-") || element.hasAttribute("data-action");
  ...
  function quickVisibilityCheck(element) {
    ...
    return element.offsetWidth > 0 && element.offsetHeight > 0 && !element.hasAttribute("hidden") && element.style.display !== "none" && element.style.visibility !== "hidden";
  }
  ...
  function buildDomTree(node, parentIframe = null) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      if (nodeData.isVisible) {
        ...
        if (nodeData.isTopElement) {
          ...
          // if (node.clientWidth !== undefined && (node.clientWidth > 5 || node.clientHeight > 5)) {
            ...
            if (doHighlightElements) {
              if (focusHighlightIndex >= 0) {
                if (focusHighlightIndex === nodeData.highlightIndex) {
                  node.setAttribute("browser_use_index", nodeData.highlightIndex.toString());
                  nodeData.attributes["browser_use_index"] = nodeData.highlightIndex.toString();
              ...
              node.removeAttribute("browser_use_index");
            }
            // }else{node.removeAttribute('browser_use_index')}
          } else {
            node.removeAttribute("browser_use_index");
          }
        } else {
    ...
    if (node.tagName) {
      ...
      // Handle regular elements
        ...
        if (node.childNodes.keys().length != 0) {
          for (const child of node.childNodes) {
            const domElement = buildDomTree(child, parentIframe);
            if (domElement) nodeData.children.push(domElement);
          }
